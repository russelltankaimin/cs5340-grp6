...
def preprocess_audio(audio_path: str, target_sr: int, device: str) -> tuple[torch.Tensor, int]:
...
def split_clips(waveform: torch.Tensor, sr: int, clip_seconds: float) -> torch.Tensor:
    ...
    clips = clipped.reshape(waveform.shape[0], num_clips, samples_per_clip).permute(1, 0, 2)
    return clips


def compute_latent_stats(model: EAR_VAE, clips: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    latents = []
    with torch.no_grad():
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Compute VAE latent mean/std stats across fixed-length audio sub-clips.')
    parser.add_argument('--audio-path', type=str, default=os.path.join('data', 'sample_2.wav'))
