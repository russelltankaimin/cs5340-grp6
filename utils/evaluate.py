...
def main(args):
    ...
    results = {"baseline_corrupted": baseline_metrics, "reconstructed": recon_metrics}
    ...
    plot_spectrograms(
        clean, corrupted, recon, args.sr, str(out_dir / "spectrogram_comparison.png")
    )

    plot_metrics_comparison(
        baseline_metrics, recon_metrics, str(out_dir / "metrics_comparison.png")
    )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--clean", type=str, required=True, help="Path to the original clean audio"
    )
    p.add_argument(
        "--corrupted", type=str, required=True, help="Path to the corrupted audio input"
    )
    p.add_argument(
        "--recon",
        type=str,
        required=True,
        help="Path to the reconstructed audio output",
    )
    p.add_argument(
        "--output-dir",
        type=str,
        default="evaluation_results",
        help="Directory to save graphs and JSON",
    )
    p.add_argument(
        "--sr", type=int, default=44100, help="Target sample rate for evaluation"
    )
